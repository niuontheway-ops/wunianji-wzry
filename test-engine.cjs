...
assert.equal(chapters.flatMap(c=>c.questions).length,224);
assert.equal(chapters.filter(c=>c.semester==='上册').length,25);
assert.equal(chapters.filter(c=>c.semester==='下册').length,23);
...
assert.equal(new Set(chapters.flatMap(c=>c.questions.map(q=>q.id))).size,224);
...
console.log('PASS: 64 themes / 224 questions, two textbook volumes, English reading, scoring, wrong-answer removal, reward deduplication, 1200 generated questions, original-text backups and 18 voice-role assets.');
